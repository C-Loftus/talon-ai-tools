...
{user.modelAlias} help$: user.gpt_help()
...
{user.modelAlias} <user.modelPrompt> [{user.modelSource}] [{user.modelDestination}]:
...
{user.modelAlias} take response: user.gpt_select_last()
...
{user.modelAlias} apply [from] clip$:
    prompt = clip.text()
...
{user.modelAlias} [nope] that was <user.text>$:
    result = user.gpt_reformat_last(text)
    user.paste(result)

# Clear the context stored in the model
{user.modelAlias} clear context: user.gpt_clear_context()
...
{user.modelAlias} clear thread: user.gpt_clear_thread()
{user.modelAlias} enable thread: user.gpt_enable_threading()
{user.modelAlias} disable thread: user.gpt_disable_threading()
